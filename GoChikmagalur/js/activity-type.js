...
$(function() {
    ...
    $('.top-load-btn').click(function(e) {
        e.preventDefault()
        $('.up-gallery .all:hidden').slice(0, 9).slideDown()
...
//       $('.'+selectedClass+', .card-lower-body ,.card-body-title, .card-body-para, .stars, .fa-star,.row, .booknow a').show()
...
function sortImages(val) {
    ...
    if (title == 'all') {
        $(function() {
            ...
            setTimeout(function() {
                $("." + selectedClass).closest('div').parent().removeAttr('data-aos').fadeIn().addClass('animation');
                $('.' + selectedClass + ', .card-lower-body ,.card-body-title, .card-body-para, .stars, .fa-star,.row, .booknow a').fadeIn()
                $("#gallery").fadeTo(300, 1);
            }, 300);
            $('.bottom-layout .col-md-6').removeAttr('data-aos', 'fade-up')
            $('.top-load-btn').show()
